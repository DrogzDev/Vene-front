...
import type { AriaAttributes, CSSProperties, PointerEvent as ReactPointerEvent, ReactNode } from "react"
import {
  animate,
  motion,
  useMotionTemplate,
...
/**
 * Botón "resortera": arrastrar y soltar dispara `onSend` con un lanzamiento
 * de partículas; un simple toque también lo dispara al instante
 * (`tapSends`, activo por defecto).
 *
 * Excepción deliberada y acotada al convenio GSAP-only del proyecto (ver
 * src/motion/motion.ts): este componente usa Framer Motion (paquete
 * `motion`) porque su física de arrastre (useMotionValue + springs +
 * Web Animations API para las partículas) no se traduce bien a GSAP puro.
 * Ningún otro componente de la app debería adoptar este patrón sin
 * discutirlo primero — el resto sigue animando con GSAP vía `withMotion`.
 */
...
type SlingButtonProps = {
  ...
  disabled?: boolean
  ariaLabel?: string
  /** Para usarlo como pestaña de navegación (p. ej. la barra inferior). */
  ariaCurrent?: AriaAttributes["aria-current"]
  className?: string
  /** Clases extra para el pomo (`.sling-button__pad`), no para la raíz. */
  padClassName?: string
}

type GripHistoryPoint = { x: number; y: number; t: number }
...
type Grip = {
  ...
  scale: number
  moved: boolean
  hist: GripHistoryPoint[]
  rawOrigin: { x: number; y: number }
  slop: number
}

interface SlingCSSVars extends CSSProperties {
...
export default function SlingButton({
  ...
  const onPointerDown = (e: ReactPointerEvent<HTMLButtonElement>) => {
    ...
    try {
      ...
      // Algunos entornos (tests, ciertos WebViews) no soportan Pointer Capture.
    }
    setHeld(true)
  }
  ...
  const release = (pointerId: number, cancelled: boolean) => {
    ...
    try {
      ...
      // Ver comentario equivalente en onPointerDown.
    }
    ...
    >
      ...
      <motion.span className="sling-button__move" style={{ transform: padT }}>
        <button
          ref={padRef}
          type="button"
          className={`sling-button__pad${padClassName ? ` ${padClassName}` : ""}`}
      ...
      <span id={hintId} className="sling-button__sr">
        Mantén y suelta para invertir, o toca para hacerlo al instante.
      </span>
    </span>
  )
}
